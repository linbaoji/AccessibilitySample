...
public class SignatureView extends View {
    // View state
    private List<TimedPoint> stroke;// 当前这一笔
    private List<List<TimedPoint>> strokeList;// 所有的笔画
...
//		mPaint.setTextSize(100);             //绘制文字大小，单位px
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                mLastTouchX = eventX;
                mLastTouchY = eventY;
                addPoint(new TimedPoint(eventX, eventY, MotionEvent.ACTION_DOWN));

            case MotionEvent.ACTION_MOVE:
                resetDirtyRect(eventX, eventY);
                addPoint(new TimedPoint(eventX, eventY, MotionEvent.ACTION_MOVE));
                break;

            case MotionEvent.ACTION_UP:
                resetDirtyRect(eventX, eventY);
                addPoint(new TimedPoint(eventX, eventY, MotionEvent.ACTION_UP));
                getParent().requestDisallowInterceptTouchEvent(true);
                setIsEmpty(false);
                break;
    ...
    private void addPoint(TimedPoint newPoint) {
        ...
        String jsonString = JSONObject.toJSONString(newPoint);

        AccessibilityLog.printLog("TimedPoint :" + jsonString);


        mPoints.add(newPoint);
        addStrokePoints(newPoint);
    ...
    private void addStrokePoints(TimedPoint newPoint) {

        if (newPoint.action == MotionEvent.ACTION_DOWN) {
            // 方下手指时 ，为新笔画
    ...
    public List<TimedPoint> getmPoints() {
        return mPoints;
    }

    public List<List<TimedPoint>> getStrokeList() {
        return strokeList;
    }
}
